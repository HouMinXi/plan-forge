...
| T23 | corpus/db.py refinements + record.py + query.py | 3 modules + integration tests | start_run / record_finding / record_evidence / record_arbitration / record_outcome / finalize_run all functional; update_evidence_tier (sole tier UPDATE, 3-way monotonic); INSERT-only except finalize_run (plan_runs completion fields, idempotent) and update_evidence_tier (llm_evidence.tier, monotonic); SQLite FK enforcement on via connect listener |
| T24 | corpus/redact.py + --corpus-private flag (DONE) | privacy module + CLI flag | redacted plan_text stored as NULL with plan_hash preserved; check() wires start_run/finalize_run; 460 tests pass |
...
| T28.5 | Wire record_finding + record_evidence into check() | check() persists findings + llm_evidence per run + integration tests | every check() run writes findings + llm_evidence rows (closes SC-16); arbitration finding_id becomes linkable; MUST precede T30 (G10 hard-depends on persisted evidence rows via update_evidence_tier); may run parallel to T29 |
...
| T31.5 | Configurable provider base_url + GLM (Zhipu) client (PLAN A) | base_url_for() in credentials.py + the 4 clients use it (Phase 1, fixes kimi); new glm_client.py @register (Phase 2) | env-unset == current full suite green (back-compat); PLAN_FORGE_KIMI_BASE_URL set -> build_active_list includes kimi (auth_ok); glm discoverable when its key resolves; rotation stays in the user proxy (registry round-robin remains deferred); MUST land before T33 dogfood + T35 ship |
